...
// Assemble the rolling main-branch development snapshot under main-build/.
...
if (!buildVersion) {
  throw new Error("BUILD_VERSION is required when packaging a main-branch development build.");
}
if (!buildSha || !/^[0-9a-f]{7,40}$/i.test(buildSha)) {
  throw new Error(
    "BUILD_SHA must be a Git commit SHA when packaging a main-branch development build.",
  );
}
...
for (const file of requiredFiles) {
  if (!fs.existsSync(file)) {
    throw new Error(`Cannot package main build: required file '${file}' does not exist.`);
  }
}
...
console.log(`Main development build packaged: ${zipPath}`);
